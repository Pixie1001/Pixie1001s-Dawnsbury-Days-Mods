    ...
    public class EchidnaditeHighPriestess {
        public static Creature Create() {
            ...
            .AddSpellcastingSource(SpellcastingKind.Prepared, Trait.Cleric, Ability.Wisdom, Trait.Divine).WithSpells(
                ...
                level3: [SpellId.SuddenBolt, SpellId.Fear, SpellId.Heal],
                level4: [SpellLoader.VomitSwarm, SpellId.Heal, SpellId.Heal]
                ).Done()
            .AddQEffect(new QEffect() {
                ProvideActionIntoPossibilitySection = (self, section) => {
                    ...
                    return (ActionPossibility) new CombatAction(self.Owner, new SideBySideIllustration(IllustrationName.PickUp, Illustrations.Javelin), "Draw javelin", [Trait.Basic, Trait.Manipulate], "",
                        Target.Self((user, ai) => !user.HasEffect(QEffectIds.AbyssalRot) && !user.HeldItems.Any(itm => itm.ItemName == CustomItems.Javelin) ? 15f : 0f)
            ...
            .AddQEffect(new QEffect() {
                ProvideActionIntoPossibilitySection = (self, section) => {
                    ...
                    return (ActionPossibility) StrikeRules.CreateStrike(self.Owner, self.Owner.HeldItems.FirstOrDefault(itm => itm.ItemName == CustomItems.Javelin)!, RangeKind.Ranged, -1, true)
            ...
            .AddQEffect(new QEffect("Monstrous Form", "When the Echidnadite High Priestess runs out of javelins or is reduced to half health, she takes on a monstrous battle form.") {
                AfterYouTakeDamage = async (self, _, _, _, _) => {
                    if (self.Owner.Damage > self.Owner.MaxHPMinusDrained)
                        Transform(self.Owner);
                },
                AfterYouTakeAction = async (self, ca) => {
            ...
            .Builder
            //.AddMainAction(you => new CombatAction(you, new SideBySideIllustration(IllustrationName.PickUp, Illustrations.Javelin), "Draw javelin", [Trait.Basic, Trait.Manipulate], "",
        ...
        private static void Transform(Creature monster) {
            ...
            .AddNaturalWeapon("tail", Illustrations.MermaidTail, 20, [Trait.Agile, Trait.Sweep], "2d8+8", DamageKind.Bludgeoning)
            .Done();
