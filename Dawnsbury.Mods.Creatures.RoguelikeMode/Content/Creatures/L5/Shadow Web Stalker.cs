...
namespace Dawnsbury.Mods.Creatures.RoguelikeMode.Content.Creatures {
    ...
    public class ShadowWebStalker {
        public static Creature Create() {
            ...
            Creature monster = new Creature(new SpiderIllustration(Illustrations.BebilithSpawn, Illustrations.Bear2), "Shadow Web Stalker",
            ...
            .AddQEffect(new QEffect("Attention Vulnerability", "When the shadow web stalker is spotted by the seek action or hit by an ability that has the Light trait or counters invisibility, it suffers 2d6 mental damage.") {
                AfterYouAreTargeted = async (self, action) => {
                    if (action?.Owner?.Occupies != null && ((action.ActionId == ActionId.Seek && action.CheckResult >= CheckResult.Success) || action.HasTrait(Trait.Light) || action.SpellId == SpellId.Glitterdust)) {
                        await CommonSpellEffects.DealDirectDamage(null, DiceFormula.FromText("2d6", "Bravery vulnerability"), self.Owner, CheckResult.Failure, DamageKind.Mental);
                    }
                }
            })
            .Builder
            .AddMainAction(you => {
                return new CombatAction(you, IllustrationName.Invisibility, "Shadow Slip", [Trait.Illusion], "The shadow web stalker becomes invisible, hides and then sneaks.",
                    Target.Self((caster, ai) => caster.Actions.ActionsLeft == 1 ? AIConstants.EXTREMELY_PREFERRED : AIConstants.NEVER))
                .WithActionCost(1)
                .WithSoundEffect(SfxName.PhaseBolt)
                .WithEffectOnSelf(async caster => {
                    caster.AddQEffect(QEffect.Invisibility());
