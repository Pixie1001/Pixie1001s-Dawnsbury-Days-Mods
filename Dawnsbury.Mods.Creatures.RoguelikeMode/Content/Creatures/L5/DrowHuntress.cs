...
namespace Dawnsbury.Mods.Creatures.RoguelikeMode.Content.Creatures {
    ...
    public class DrowHuntress {
        public static Creature Create() {
            var monster = new Creature(Illustrations.DrowSniper, "Drow Huntress", new List<Trait>() { Trait.Chaotic, Trait.Evil, Trait.Elf, ModTraits.Drow, Trait.Humanoid, ModTraits.ArcherMutator }, 5, 15, 6, new Defenses(21, 9, 15, 12), 55,
            ...
            .AddQEffect(new QEffect("Shadowstep {icon:Reaction}", "{b}Trigger{/b} The drow huntress is damaged by an attack. {b}Effect{/b} The drow huntress teleports to an unoccupied space up to 30ft away.") {
                AfterYouTakeDamage = async (self, amount, kind, action, critical) => {
                    ...
                    if (await self.Owner.AskToUseReaction("Use Shadowstep to teleport up to 30ft away.")) {
                        ...
                        foreach (Tile tile in self.Owner.Battle.Map.AllTiles.Where(t => t.IsTrulyGenuinelyFreeToEveryCreature && !t.HazardousTerrainEphemeral && t.DistanceTo(self.Owner.Occupies) <= 6)) {
                            int score = 0;
                            foreach (Creature enemy in self.Owner.Battle.AllCreatures.Where(cr => !cr.FriendOf(self.Owner))) {
