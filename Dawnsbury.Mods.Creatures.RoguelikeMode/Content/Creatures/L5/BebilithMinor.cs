            ...
            .AddQEffect(new QEffect() {
                StateCheckWithVisibleChanges = async self => {
                    if (self.Owner.Battle.Encounter.CharacterLevel <= 3 && CampaignState.Instance != null && CampaignState.Instance.AdventurePath?.Name == "Roguelike Mode" && CampaignState.Instance.Tags.TryGetValue("SeenBebilithSpawn", out string val) == false) {
                        ...
                        var advisor1 = UtilityFunctions.ChooseAtRandom(self.Owner.Battle.AllCreatures.Where(cr => cr.PersistentCharacterSheet != null).ToArray());
                        var advisor2 = UtilityFunctions.ChooseAtRandom(self.Owner.Battle.AllCreatures.Where(cr => cr.PersistentCharacterSheet != null && cr != advisor1).ToArray());
