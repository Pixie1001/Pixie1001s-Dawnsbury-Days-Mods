    ...
    public class EchidnaditePriestess {
        public static Creature Create() {
            ...
            .AddSpellcastingSource(SpellcastingKind.Prepared, Trait.Cleric, Ability.Wisdom, Trait.Divine).WithSpells(
                ...
                level2: [SpellId.Heal, SpellId.Heal, SpellId.Heal],
                level3: [SpellLoader.VomitSwarm, SpellLoader.VomitSwarm],
                level4: [SpellLoader.SummonMonster]).Done()
            .Builder
            ...
            .AddMainAction(you => new CombatAction(you, new SideBySideIllustration(IllustrationName.Bless, IllustrationName.Jaws), "Monstrous Gift", [Trait.Flourish, Trait.Divine], "Target echidnadite cultist within 30 feet gains a random monstrous gift.",
            ...
            .WithEffectOnEachTarget(async (action, user, target, _) => {
                ...
                var effect = new QEffect("Monstrous Gift ", "", ExpirationCondition.ExpiresAtStartOfSourcesTurn, user, IllustrationName.MagicHide);
                ...
                switch (rand) {
                    case 0:
                        ...
                        effect.Description = "You gain a +2 bonus to AC.";
                    ...
                    case 1:
                        effect.Name += "(Quills)";
                        effect.Description = "You can gain a ranged quills attacks.";
                        effect.Illustration = Illustrations.Quills;
                        effect.BattleformMinimumStrikeModifier = 15;
                    ...
                    case 2:
                        ...
                        effect.Description = "Deadly grabbing limbs protrude from your back.";
                        ...
                        effect.StateCheck = self => {
                            self.Owner.AddQEffect(QEffect.MonsterGrab(true));
                        };
                        break;
                    case 3:
                        effect.Name += "(Striking Tail)";
                        effect.Description = "As a reaction {icon:Reaction}, you may make a striking tail strike attack against creatures within reach that attack you.";
                        effect.Illustration = IllustrationName.Tail;
                        effect.AdditionalUnarmedStrike = new Item(IllustrationName.Tail, "striking tail", [Trait.Agile, Trait.Finesse, Trait.BattleformAttack])
                            .WithWeaponProperties(new WeaponProperties("2d8", DamageKind.Piercing));
                        effect.BattleformMinimumStrikeModifier = 15;
                        effect.YouAreTargeted = async (self, action) => {
                            ...
                            if ((bool)strike.CanBeginToUse(self.Owner) && (strike.Target as CreatureTarget)!.IsLegalTarget(self.Owner, action.Owner)
                                .CanBeUsed && await self.Owner.AskToUseReaction($"{action.Owner.Name} is attempting to attack you in melee. Would you like to retaliate with striking tail?")) {
