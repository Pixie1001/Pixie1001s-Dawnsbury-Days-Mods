    ...
    internal static class SkillChallengeTables {
        ...
        public static void LoadSkillChallengeTables() {
            ...
            events.Add(new SkillChallenge("Cursed Relic", async (level, battle) => {
                ...
                await battle.Cinematics.NarratorLineAsync($"Regardless the {cursedItem.Name} emanates a dark and terrible aura, no doubt possessed of a great demonic taint. And yet, the party can hardly afford to be picky in times such as these...");
                ...
                var choice = await CommonQuestions.OfferDialogueChoice(GetParty(battle).First(), GetNarrator(),
                    ...
                    $"{opt1.printInfoTag()} Have {opt1.Nominee.Name} perform a ritual to cleanse the {cursedItem.Name} of its curse.",
                    $"{opt2.printInfoTag()} {opt2.Nominee.Name} believes they might be able to find a loophole in the curse.",
                    $"{opt3.printInfoTag()} {opt3.Nominee.Name} hesitantly suggests that the demon might yet still be bargained with to bequeash the {cursedItem.Name} to the party.",
                ...
                switch (choice.Index) {
                    case 0:
                        await battle.Cinematics.NarratorLineAsync($"{opt1.Nominee.Name} spends several hours setting up an elaborate ritual to cleanse the {cursedItem.Name}.");
                        result = opt1.Roll();
                        if (result >= CheckResult.Success) {
                            await battle.Cinematics.NarratorLineAsync(PrintResult(result) + $"Heavenly light bathes the {cursedItem.Name}, banishing the evil energy lurking within!");
                            battle.CampaignState.CommonLoot.Add(cursedItem);
                        } else {
                            await battle.Cinematics.NarratorLineAsync(PrintResult(result) + $"{opt1.Nominee.Name}'s attempts to cleanse the item do not go unnoticed. Barely a moment passes before the {cursedItem.Name} explodes into " +
                        ...
                        break;
                    case 1:
                        await battle.Cinematics.NarratorLineAsync($"{opt2.Nominee.Name} spends several hours consulting a collection of heavy grimoires and leather bound volumes in order to identify and circumvent the maladiction placed on the {cursedItem.Name}.");
                        result = opt2.Roll();
                        if (result >= CheckResult.Success) {
                            await battle.Cinematics.NarratorLineAsync(PrintResult(result) + $"{opt2.Nominee.Name} emerges with the {cursedItem.Name} several hours later, now bound in rune scribed binding wraps, before quickly running the rest of their through how to safely operate it without activating the curse.");
                            await battle.Cinematics.NarratorLineAsync($"The {cursedItem.Name} should be safe for the party to use now... Probably. Though it's unlikely any merchant will want to take it.");
                            cursedItem.Price = 0;
                            ...
                            await battle.Cinematics.NarratorLineAsync(PrintResult(result) + $"After severa labourious hours, {opt2.Nominee.Name} tenetatively reaches out to put their theory to the test... Holding it aloft for several promising moment passes before the {cursedItem.Name} abruptly explodes into " +
                                $"a cloud of sickly black smoke and violently drives itself down {opt1.Nominee.Name}'s throat!");
                            await battle.Cinematics.NarratorLineAsync($"Though the party manages to rouse {opt1.Nominee.Name} several hours later, they appear afflicted by a sickly pallour.");
                            await battle.Cinematics.NarratorLineAsync($"{opt1.Nominee.Name} has been cursed with Clumsy, Enfeebled and Stupified 1 until their next long rest.");
                            opt1.Nominee.LongTermEffects.Add(WellKnownLongTermEffects.CreateLongTermEffect("Lingering Curse"));
                    ...
                    case 2:
                        ...
                        result = opt3.Roll();
                        if (result >= CheckResult.Success) {
                            await battle.Cinematics.NarratorLineAsync(PrintResult(result) + $"A series of complicated clauses and entreaties soon follow as {opt3.Nominee.Name} negotiates with sinister, lascivious voice eminating from the {cursedItem.Name}.");
                        ...
                        break;
                    case 3:
                        await battle.Cinematics.NarratorLineAsync($"With one last look at the ominous smoke curling off the {cursedItem.Name} the party wisely moves on ahead, leaving the accursed item untouched.");
                        break;
                }
            }));
