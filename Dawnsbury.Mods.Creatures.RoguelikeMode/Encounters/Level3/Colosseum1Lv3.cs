...
namespace Dawnsbury.Mods.Creatures.RoguelikeMode.Encounters.Level3
{
    ...
    internal class Colosseum1Lv3 : Level3Encounter
    {
        public Colosseum1Lv3(string filename) : base("Colosseum", filename)
        {
            ...
            ReplaceTriggerWithCinematic(TriggerName.AllEnemiesDefeated, async (TBattle battle) =>
            {
                if (round == 1)
                {
                    if (await AskToContinueBattle(battle, round))
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            var creature = Bodyguard.Create().ApplyEliteAdjustments(false);
                            creature.MainName = "Gladiator";
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            battle.SpawnCreature(Pikeman.Create().ApplyEliteAdjustments(false), battle.Enemy, 6 + i % 2, 2);
                        }
                ...
                else if (round == 2)
                {
                    if (await AskToContinueBattle(battle, round))
                    {
                        ...
                        battle.SpawnCreature(champion.ApplyEliteAdjustments(false), battle.Enemy, 7, 2);
